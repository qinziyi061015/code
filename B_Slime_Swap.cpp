...
int main(){
    ...
    auto weights = [&](int i){
        ...
        if(A[i]==-1){
            ...
            if(hasL && A[i]==i-1) wL=1;
            else if(hasR && A[i]==i+1) wR=1;
            else wO=1;
        }
    ...
    for(int i=1;i<=N-1;i++){
        ...
        // From dp0: free at i
        if(dp0){
            if(wR) ndp0 = addmod(ndp0, mulmod(dp0, wR)); // choose R -> satisfy pair i
            int to1 = (wL + wO) % MOD; // choose L or O -> need force x_{i+1}
            if(to1) ndp1 = addmod(ndp1, mulmod(dp0, to1));
        }
    ...
    if(dp0){
        ...
        if(sum) ans = addmod(ans, mulmod(dp0, sum));
    }
    if(dp1){
        if(wLN) ans = addmod(ans, mulmod(dp1, wLN)); // must choose L
    }
